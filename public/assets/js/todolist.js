window.initTodoList = () => {
	...
	if (!todoList || todoList.dataset.initialized) return;
	todoList.dataset.initialized = "true";
	...
	jQuery('#todoAdd').unbind('click').on('click', function(){
	...
	$('#todoList').unbind('change').on('change', '.form-check-input', function () {
	...
	if (todoListSortable && typeof Sortable !== 'undefined') {
	...
	$('#todoList').unbind('click').on('click', '.item-delete', function(){
		$(this).parent().remove();
	});
};

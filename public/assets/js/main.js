const initAppToggler = () => {
	...
	appTogglers.forEach(toggler => {
		if (toggler.dataset.initialized) return;
	...
	if (!appMenubars.dataset.initialized) {
		appMenubars.dataset.initialized = "true";
...
const saerchList = () => {
	...
	if (searchInput.length === 0 || searchInput.data('initialized')) return;
	searchInput.data('initialized', true);
...
function initTooltips() {
	...
	tooltipTriggerList.forEach(tooltipTriggerEl => {
		if (tooltipTriggerEl.dataset.initialized) return;
...
function initPopover() {
	...
	popoverTriggerList.forEach(popoverTriggerEl => {
		if (popoverTriggerEl.dataset.initialized) return;
		popoverTriggerEl.dataset.initialized = "true";
...
function initSidebarMenu() {
	...
	const nk = window.location.pathname;
	$(".app-navbar .menubar a").each(function() {
		// match exact or partial pathname
		const href = $(this).attr('href');
		if (href && href !== '#' && !href.startsWith('javascript')) {
			const cleanHref = href.split('?')[0].split('#')[0];
			if (nk === cleanHref || (nk === '/' && cleanHref === '/')) {
				$(this).addClass("active");
				$(this).parent().addClass("active");
				$(this).parent().parent().show().siblings('a').addClass("active open");
				$(this).parent().parent().parent().parent().show().siblings('a').addClass("open");
			}
		}
	});
}
...
function initBookmarks() {
	if (document.datasetBookmarkedInitialized) return;
	document.datasetBookmarkedInitialized = true;
...
const ThemeSwitcher = () => {
	...
	if (!window.themeSwitcherMediaListened) {
		window.themeSwitcherMediaListened = true;
	...
	window.reinitThemeSwitcher = () => {
		const prefTheme = getPreferredTheme();
		setTheme(prefTheme);
		showActiveTheme(prefTheme);
		bindThemeButtons();
	};
};

function initSidebarPanel() {
	if (document.datasetSidebarPanelInitialized) return;
	document.datasetSidebarPanelInitialized = true;
...
window.reinitTemplateLayout = () => {
	if (typeof Waves !== 'undefined') {
		try { Waves.init(); } catch (e) {}
	}
	initAppToggler();
	saerchList();
	setElementHeight();
	...
	if (typeof window.reinitThemeSwitcher === 'function') {
		window.reinitThemeSwitcher();
	} else {
		ThemeSwitcher();
	}
